...
def update_optim_state(optim: Optimizer, gaussians: data.Gaussians3D, old_state_input_indices: torch.Tensor, new_state_output_indices: torch.Tensor):
     new_datas = [
          gaussians.means,
          gaussians.rotations,
          gaussians.scales,
...
def main():
     ...
     slang_device = render.create_slangpy_device_for_torch(optimization_level=args.optim, fp_mode=args.fp_mode, include_paths=[str(Path(__file__).parent)])
     adc_module = spy.Module.load_from_file(slang_device, "adc.slang")
     ...
     gaussians = data.Gaussians3D.from_colmap(rec_path, workers=args.workers if (args.workers == -1 or args.workers > 0) else 1, autograd=True)
     ds = data.CalibratedImages.from_colmap(rec_path, str(args.reconstruction / "images"))
     train_ds, _ = ds.split_train_test()

     ctx = render.RenderContext.from_settings(gaussians.num, args.tile_size, args.block_size, render_module, ds.max_image_size)
     backprop_data = render.BackpropagationData.from_settings(gaussians.num, ds.max_image_size)
     densif_data = render.DensificationData.dummy()
     opts = render.RenderOptions(max_sh_degree=args.starting_sh_degrees, background_color=args.bg_color, save_data_for_backprop=True, collect_data_for_densification=first_densification_iter<=0)
     max_width, max_height = ds.max_image_size
     out_image = torch.zeros((max_height, max_width, 3), dtype=torch.float32, device="cuda")

     density_control = adc.AdaptiveDensityControl.from_settings(
          gaussians.num,
          adc_module,
          args.block_size,
     ...
     for factor, iters in zip(args.factors, args.factors_iters):
          ds.transform = v2.Compose([data.DownsampleCalibratedImage(factor), v2.ToDtype(torch.float32, scale=True)]) if factor > 1 else v2.ToDtype(torch.float32, scale=True)
          ...
          while curr_iter < iters:
               for gt_image, intrinsics, hfsc, extrinsics in dl:
                    ...
                    cam = render.Camera((w,h), intrinsics, hfsc, extrinsics)
                    out_image_view = out_image[:h,:w,:]
                    if is_bg_rand:
                         opts.background_color = torch.rand(3, device="cuda")
                    render.render(gaussians, cam, ctx, opts, out_image_view, backprop_data, densif_data)
                    ...
                    if is_densification_enabled:
                         if opts.collect_data_for_densification:
                              ...
                              if curr_iter % args.densification_interval == 0:
                                   ...
                                   print(f"ViewCounters Min {densif_data.view_counters.min()} Max {densif_data.view_counters.max()}")
